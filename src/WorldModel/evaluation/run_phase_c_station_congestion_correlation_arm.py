"""Run one frozen policy arm with same-tick congestion instrumentation."""
...
def _load_diagnostic_bundle(path: Path) -> tuple[dict, dict]:
    ...
    if bundle.get("schema_version") != FROZEN_BUNDLE_SCHEMA_VERSION:
        raise ValueError(f"wrong station congestion bundle schema: {path}")
    protocol = bundle.get("protocol")
    if not isinstance(protocol, Mapping):
        raise ValueError("station congestion bundle lacks protocol")
    protocol = dict(protocol)
    claimed = str(protocol.pop("protocol_sha256", ""))
    if protocol.get("schema_version") != SCHEMA_VERSION:
        raise ValueError("wrong station congestion protocol schema")
    if canonical_sha256(protocol) != claimed:
        raise ValueError("station congestion protocol hash mismatch")
...
def _verify_bundle_artifact(bundle: Mapping[str, Any], key: str) -> Path:
    ...
    if not isinstance(value, Mapping):
        raise ValueError(f"station congestion bundle lacks artifact {key!r}")
    path = Path(str(value.get("path", "")))
    expected = str(value.get("sha256", ""))
    if not path.is_file():
        raise FileNotFoundError(path)
    if sha256_file(path) != expected:
        raise ValueError(f"station congestion frozen artifact changed: {path}")
    return path
...
def _write_output_hashes(root: Path) -> Path:
    ...
    files = sorted(
        ...
        if path.is_file()
        and path.name not in {"station_congestion_summary.json", output.name}
    )
    rows = [
...
def _resume(
    ...
    *,
    ...
    summary_path = run_dir / "station_congestion_summary.json"
    ...
    output_manifest = run_dir / "run_outputs.sha256"
    checks["output_manifest"] = (
        output_manifest.is_file()
        and sha256_file(output_manifest) == summary.get("run_outputs_sha256")
    )
    checks["outputs"] = _verify_output_hashes(run_dir, output_manifest)
...
def main() -> None:
    ...
    parser.add_argument(
        "--frozen-bundle",
        default=str(OUTPUT_ROOT / "phase_c_station_congestion_frozen_protocol.json"),
    )
    ...
    if int(args.seed) not in SEEDS:
        raise SystemExit(f"seed must be one of the frozen development seeds {list(SEEDS)}")
    rid = run_id(args.arm, args.load, args.seed)
    bundle_path = Path(args.frozen_bundle)
    diagnostic_bundle, protocol = _load_diagnostic_bundle(bundle_path)
    ...
    expected_source_bundle = _verify_bundle_artifact(
        diagnostic_bundle, "source_frozen_bundle"
    )
    ...
    config_path = Path(str(_artifact(
        source_bundle, f"config_{args.load}"
    )["path"]))
    ...
    if args.arm != "greedy" and not manifest_path.is_file():
        raise FileNotFoundError(
            f"Greedy manifest must finish before {args.arm}: {manifest_path}"
        )
    if run_dir.exists():
    ...
    checks = {
        "fresh_development_seed": int(args.seed) in SEEDS,
    ...
    if not audit["passed"]:
        failed = [key for key, passed in checks.items() if not passed]
        raise RuntimeError(f"station congestion run audit failed: {failed}")
    ...
    _atomic_write_json(staging / "station_congestion_summary.json", summary)
