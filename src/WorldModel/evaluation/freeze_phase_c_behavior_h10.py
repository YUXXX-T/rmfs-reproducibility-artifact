"""Freeze the independent behavior-continuation H=10 test bundle."""
...
FROZEN_FILENAME = "phase_c_behavior_h10_frozen_protocol.json"
...
def main() -> None:
    ...
    for load in ("low", "mid", "high"):
        row = artifacts["load_configs"][load]
        rows.append(f"{row['sha256']}  {row['path']}")
    ...
    print(f"behavior H=10 protocol sha256 = {protocol_sha}")
    print(f"seeds = {protocol['inputs']['seeds']}")
    print(f"loads = {protocol['inputs']['loads']}")
