"""Run one resumable arm of the 541--550 phi-context ablation."""
...
def _load_bundle(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if bundle.get("schema_version") != FROZEN_BUNDLE_SCHEMA_VERSION:
        raise ValueError(f"wrong phi-context frozen bundle: {path}")
    protocol = bundle.get("protocol")
    if not isinstance(protocol, Mapping):
        raise ValueError("phi-context bundle lacks protocol")
...
def _make_assigner(
    ...
    *,
    ...
    if arm == S1_PHI_ARM:
        return PhiContextRankWorldModelTaskAssigner(
            phi_head_checkpoint=str(phi_head_checkpoint),
            phi_scale_contract=str(phi_scale_contract),
            phi_context_mode="service_ascending",
            phi_context_factor=PHI_CONTEXT_FACTOR,
            phi_trace_enabled=True,
            phi_trace_max_records=TICKS,
            **common,
        )
...
def _audit(
    ...
    *,
    ...
    if arm == S1_PHI_ARM:
        checks.update({
            "phi_head_loaded": bool(
                metrics.get("phi_context_head_loaded", False)
            ),
            "phi_evaluated": int(metrics.get("phi_context_eval_calls", 0)) > 0,
            "phi_pressure_is_nonincreasing": float(metrics.get(
                "phi_context_applied_budget_pressure_mean", 0.0
            )) <= float(metrics.get(
                "phi_context_baseline_budget_pressure_mean", 0.0
            )) + 1e-8,
            "phi_no_hard_gate": not bool(
                metrics.get("phi_context_hard_gate_added", True)
            ),
            "phi_no_assign_not_added": not bool(
                metrics.get("phi_context_no_assign_added", True)
            ),
...
def main() -> None:
    ...
    phi_head_checkpoint = _artifact(bundle, "phi_head_checkpoint")
    phi_scale_contract = _artifact(bundle, "phi_scale_contract")
    ...
    assigner = _make_assigner(
        ...
        phi_head_checkpoint=phi_head_checkpoint,
    ...
    if isinstance(assigner, PhiContextRankWorldModelTaskAssigner):
        metrics.update(assigner.phi_context_metrics())
    ...
    if not audit["passed"]:
        ...
        raise RuntimeError(
            f"phi-context arm audit failed {args.arm} {args.load} "
    ...
    payload = {
        ...
        "meta": {
            ...
            "phi_head_checkpoint": (
                phi_head_checkpoint.as_posix()
                if args.arm == S1_PHI_ARM else None
            ),
        },
        "manifest": {
