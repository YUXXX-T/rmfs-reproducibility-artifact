"""Six-station 20x20/48-robot World-Model *adaptation* campaign (formal).

This is a controlled cross-map generalisation experiment.  It reuses the exact
stage recipe of the audited four-station -> six-station adaptation campaign
(``run_phase_c_station6_adaptation.py``) but retargets it at the hand-authored
20x20 / 48-robot / 6-station configuration files:

    Config/world_model_config_PP_48_{low,mid,high}_6stations.json

Relative to the frozen 30x30/72-robot campaign this holds map size AND robot
count fixed and changes ONLY the station count / layout (4 -> 6).  That makes
the model-side change minimal and clean: the demand channel count goes 4 -> 6
(``demand_dim`` 9 -> 11) via the same grid-size-agnostic adapter; every other
tensor and the grid itself are untouched.  It is therefore the strongest
"new station configuration" controlled comparison available in this repo.

Pipeline (identical dependency order to the audited campaign):

1. prepare        : validate the 6-station geometry, freeze the load configs,
                    and build the 4->6 demand-schema adapter;
2. collect_base   : Greedy counterfactual data on the 6-station map;
3. train_base     : train the six-station behaviour WM from the adapter;
4. collect_core   : pure WM-on-policy snapshots + isolated H=10 counterfactual
                    datasets  (this is the on-policy counterfactual adaptation
                    "OCA" data);
5. train_core     : train the six-station Phase-C core (OCA);
6. collect_repair : seed-disjoint snapshots from the trained core + W-step
                    LongRisk labels  (the long-risk head data);
7. train_repair   : LongRiskHead-only repair + state-level station (J1) head;
8. evaluate       : paired Greedy / Hungarian / Phase-C / Combo-S1+J1 online
                    tests on held-out seeds, then summarize.

The final Combo arm is an *adapted* six-station model, never a frozen
four-station zero-shot result.  Core-adaptation seeds, held-out online seeds,
and LongRiskHead/J1 seeds are mutually disjoint.

------------------------------------------------------------------------------
FORMAL SCALE.  The protocol below uses the audited full seed blocks
711-720 / 721-730 / 731-740 and TICKS=1500.
------------------------------------------------------------------------------

This runner does not modify the simulator, graph builder, task assigners,
station queues, or training code.  Its stages are resumable and leave a
complete lineage.
"""
...
SCHEMA_VERSION = "station6_20x20_adaptation_protocol_v1"
RUN_SCHEMA_VERSION = "station6_20x20_adaptation_run_v1"
SUMMARY_SCHEMA_VERSION = "station6_20x20_adaptation_summary_v1"

BASE_ROOT = Path("WorldModel/checkpoints/station6_20x20_adaptation_v1")
DEFAULT_OUTPUT_ROOT = BASE_ROOT / "full_711_740_v1"

# 4-station / demand_dim=9 adapter source (same lineage as the frozen campaign).
...
# The hand-authored 20x20 / 48-robot / 6-station configs are read directly.
SOURCE_CONFIG_PATTERN = "Config/world_model_config_PP_48_{load}_6stations.json"

LOADS = ("low", "mid", "high")

# ===================== SCALE-UP KNOBS (formal values) ====================
# Formal six-station 20x20/48-robot protocol:
#   CORE_DATA_SEEDS   = 711-720; train 711-717, val 718-719, test 720
#   REPAIR_DATA_SEEDS = 731-740; train 731-737, val 738-739, test 740
#   ONLINE_TEST_SEEDS = 721-730; TICKS = 1500
...
def _run(
    ...
    *,
    ...
    if log is None:
        subprocess.run(list(command), cwd=str(cwd), check=True, env=child_env)
        return
...
def _cpu_training_env(max_threads: int = 60) -> dict[str, str]:
    ...
    requested = int(
        os.environ.get("STATION6_20X20_TRAIN_THREADS") or allocated
    )
...
def _protocol_path(root: Path) -> Path:
    return root / "station6_20x20_adaptation_protocol.json"
...
def _validate_geometry(config_path: Path) -> dict[str, Any]:
    """Bake the '合理才能使用' geometry gate into the pipeline.

    Builds the real MapState + StationState and asserts the six-station
    20x20/48-robot layout is sound: correct dimensions, all station cells in
    bounds, no cross-station cell collision, every service entry walkable and
    reachable, and a fully connected walkable graph.
    """
    ...
    ms = MapState(cfg.map)
    ss = StationState(cfg, ms)

    def walkable(r: int, c: int) -> bool:
        return ms.in_bounds(r, c) and ms.grid[r][c] not in block

    # Fully-connected walkable graph from the first walkable cell.
    seed_cell = None
    ...
    if seed_cell is not None:
        reachable.add(seed_cell)
        dq = deque([seed_cell])
        while dq:
            r, c = dq.popleft()
            for dr, dc in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                nr, nc = r + dr, c + dc
                if walkable(nr, nc) and (nr, nc) not in reachable:
                    reachable.add((nr, nc))
                    dq.append((nr, nc))
    total_walkable = sum(
        1 for r in range(ms.rows) for c in range(ms.cols) if walkable(r, c)
    )

    occupied: dict[tuple[int, int], int] = {}
    collisions: list[str] = []
    entries_ok = True
    for sid in sorted(ss.stations):
        sq = ss.stations[sid]
        cells = {sq.service.position}
        cells |= {s.position for s in sq.queue_slots}
        cells |= {s.position for s in sq.buffer_slots}
        for cell in cells:
            if not ms.in_bounds(*cell):
                collisions.append(f"station {sid} cell {cell} OOB")
            if cell in occupied:
                collisions.append(
                    f"station {sid} cell {cell} collides with station {occupied[cell]}"
                )
            occupied[cell] = sid
        entry = sq.entry_position
        if entry is None or not walkable(*entry) or entry not in reachable:
            entries_ok = False
            collisions.append(f"station {sid} entry {entry} not walkable/reachable")

    checks = {
        "dimensions": ms.rows == MAP_ROWS and ms.cols == MAP_COLS,
        "robots": int(cfg.robots.num_robots) == ROBOT_COUNT,
        "station_count": len(ss.stations) == STATION_COUNT,
        "no_cross_station_collision": not collisions,
        "entries_walkable_reachable": entries_ok,
        "fully_connected": total_walkable > 0 and len(reachable) == total_walkable,
    }
    report = {
        "config": config_path.as_posix(),
        "rows": ms.rows,
        "cols": ms.cols,
        "robots": int(cfg.robots.num_robots),
        "stations": len(ss.stations),
        "walkable_cells": total_walkable,
    ...
    if not report["passed"]:
        raise ValueError(f"six-station geometry validation failed: {report}")
    return report
...
def _prepare(root: Path, ticks: int = TICKS) -> dict[str, Any]:
    ...
    for load in LOADS:
        source_path = Path(SOURCE_CONFIG_PATTERN.format(load=load))
        if not source_path.is_file():
            raise FileNotFoundError(source_path)
        source = _read_json(source_path)
        # The hand-authored 6-station config is used verbatim; there is no
        # programmatic derivation.  Validate its geometry before freezing.
        geometry[load] = _validate_geometry(source_path)
        target = _config_path(root, load)
        _atomic_json(target, source)
        load_configs[load] = {
    ...
    protocol = {
        ...
        "profile": "formal",
        "purpose": (
            "controlled cross-map six-station adaptation on 20x20/48-robot; "
            "not frozen zero-shot generalisation"
        ),
        "target": {
            "map_rows": MAP_ROWS,
            "map_cols": MAP_COLS,
        ...
        "source_config_pattern": SOURCE_CONFIG_PATTERN,
        ...
        "adapter": {
            ...
            "new_columns": "two extra station channels initialized to mean of original four",
            "all_other_tensors": "must remain bitwise equal in adapter",
            "grid_size_agnostic": True,
        },
        "domain_adaptation": {
...
def _load_protocol(root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if bundle.get("protocol", {}).get("schema_version") != SCHEMA_VERSION:
        raise ValueError("unexpected station6-20x20 protocol schema")
    protocol = bundle["protocol"]
    if _canonical_sha(protocol) != bundle.get("protocol_sha256"):
        raise ValueError("station6-20x20 protocol hash mismatch")
    if tuple(protocol.get("core_data_seeds", ())) != CORE_DATA_SEEDS:
        raise ValueError("core-data seed block changed")
    if (
        tuple(protocol.get("core_train_seeds", ())) != CORE_TRAIN_SEEDS
        or tuple(protocol.get("core_val_seeds", ())) != CORE_VAL_SEEDS
        or tuple(protocol.get("core_offline_test_seeds", ())) != CORE_TEST_SEEDS
    ):
        raise ValueError("core train/val/test split changed")
    if tuple(protocol.get("online_test_seeds", ())) != ONLINE_TEST_SEEDS:
        raise ValueError("online-test seed block changed")
    if tuple(protocol.get("repair_data_seeds", ())) != REPAIR_DATA_SEEDS:
        raise ValueError("repair-data seed block changed")
    if (
        tuple(protocol.get("repair_train_seeds", ())) != REPAIR_TRAIN_SEEDS
        or tuple(protocol.get("repair_val_seeds", ())) != REPAIR_VAL_SEEDS
        or tuple(protocol.get("repair_offline_test_seeds", ())) != REPAIR_TEST_SEEDS
    ):
        raise ValueError("repair train/val/test split changed")
    if (
        set(CORE_TRAIN_SEEDS) | set(CORE_VAL_SEEDS) | set(CORE_TEST_SEEDS)
    ) != set(CORE_DATA_SEEDS):
        raise ValueError("core split does not cover its data seeds")
    if (
        set(REPAIR_TRAIN_SEEDS) | set(REPAIR_VAL_SEEDS) | set(REPAIR_TEST_SEEDS)
    ) != set(REPAIR_DATA_SEEDS):
        raise ValueError("repair split does not cover its data seeds")
    all_data_seeds = set(CORE_DATA_SEEDS) | set(REPAIR_DATA_SEEDS)
    if all_data_seeds & set(ONLINE_TEST_SEEDS):
        raise ValueError("online-test seeds overlap adaptation data")
    if set(CORE_DATA_SEEDS) & set(REPAIR_DATA_SEEDS):
        raise ValueError("core and repair seed blocks overlap")
    if not bool(protocol.get("online_test_disjoint_from_all_data")):
        raise ValueError("protocol does not certify held-out online seeds")
    if not bool(protocol.get("core_and_repair_data_disjoint")):
        raise ValueError("protocol does not separate core/repair data")
    for load in LOADS:
...
def _write_adapter(root: Path) -> None:
    """Create and audit the 4->6 demand-schema adapter (grid-size agnostic)."""
    ...
    if old_weight is None or tuple(old_weight.shape)[1] != 9:
        raise ValueError(
            f"unexpected demand encoder tensor: {key} {getattr(old_weight, 'shape', None)}"
        )
    new_weight = torch.empty((old_weight.shape[0], 11), dtype=old_weight.dtype)
    new_weight[:, :9] = old_weight
    # Input order is [five global channels, station_1..station_S].  A neutral
    # mean initialization avoids privileging either new station.
...
def _snapshot_manifest_path(root: Path, stage: str, load: str, seed: int) -> Path:
    if stage not in ("core", "repair"):
        raise ValueError(stage)
    return root / f"{stage}_manifests" / load / f"orders_{load}_seed{seed}.json"


def _eval_manifest_path(root: Path, load: str, seed: int) -> Path:
    return root / "evaluation" / "manifests" / load / f"orders_{load}_seed{seed}.json"
...
def _run_base_cell(root: Path, load: str, seed: int, ticks: int = TICKS) -> None:
    ...
    run_id = f"station6_20x20_base_{load}_seed{seed}"
    ...
    _atomic_json(gen_config_path, {
        "schema_version": "station6_20x20_base_collection_v1",
        "run_id": run_id,
        "seed": int(seed),
        "load_level": load,
        "stations": STATION_COUNT,
    ...
    checks = {
        ...
        "no_committed_cap_contract": station_audit.get("committed_capacity_contract")
...
def _train_base(root: Path) -> Path:
    ...
    if checkpoint.is_file() and (model_root / "train_summary.json").is_file():
        if not lineage_path.is_file():
            raise FileNotFoundError(f"completed base checkpoint lacks lineage: {lineage_path}")
        lineage = _read_json(lineage_path)
        if (
            lineage.get("checkpoint_sha256") == _sha256_file(checkpoint)
            and lineage.get("source_adapter_sha256") == _sha256_file(_adapter_path(root))
        ):
            return checkpoint
    ...
    _run(command, cwd=_repo_root(), log=root / "logs" / "train_base.log", env_overrides=_cpu_training_env())
    checkpoint = model_root / "best_regret_world_model.pt"
    if not checkpoint.is_file():
        checkpoint = model_root / "world_model.pt"
    if not checkpoint.is_file():
...
def _snapshot_stage_contract(root: Path, stage: str) -> tuple[tuple[int, ...], Path, str, str]:
    if stage == "core":
        return (CORE_DATA_SEEDS, _train_base(root), "Station6PhaseCCoreBehavior", "station6_phase_c_core")
    if stage == "repair":
        return (REPAIR_DATA_SEEDS, _train_phasec_core(root), "Station6PhaseCRepairBehavior", "station6_phase_c_repair")
    raise ValueError(stage)


def _run_snapshot_cell(root: Path, stage: str, load: str, seed: int, ticks: int = TICKS) -> None:
    bundle, protocol = _load_protocol(root)
    _require_protocol_ticks(protocol, ticks)
    allowed_seeds, checkpoint, trace_label, phase_round = _snapshot_stage_contract(root, stage)
    if int(seed) not in allowed_seeds:
        raise ValueError(f"{stage} snapshot seed must be in {list(allowed_seeds)}")
    ...
    if manifest.exists():
        raise FileExistsError(f"orphan snapshot manifest exists without a completed run: {manifest}")
    ...
    run_id = f"station6_20x20_{stage}_{load}_seed{seed}"
    with _physical_only_audit_contract() as holder:
    ...
    if snapshot_files:
        ...
        snapshot_schema_checks = {
            "six_stations": int(torch.as_tensor(first_snapshot["station_node_ids"]).numel()) == STATION_COUNT,
            "demand_dim_11": int(torch.as_tensor(first_snapshot["demand_context"]).numel()) == 11,
            "world_model_on_policy": first_snapshot.get("training_source_policy") == "world_model_on_policy",
            "external_baselines_excluded": not bool(first_snapshot.get("external_baseline_training_samples")),
        }
    checks = {
        ...
        "manifest_generated_by_world_model_run": not bool(metrics.get("order_arrival_replayed")),
        "manifest": metrics.get("order_arrival_manifest_sha256") == manifest_payload.get("manifest_sha256"),
        "station_audit_passed": bool(station_audit.get("passed")),
        "physical_only": _physical_only_mode_ok(station_audit),
        "physical_capacity_no_violation": int(station_audit.get("physical_capacity_violation_count", -1)) == 0,
        "no_committed_cap_contract": station_audit.get("committed_capacity_contract")
        == "not enforced for in-transit commitments",
        **snapshot_schema_checks,
    }
    ...
    print(f"[complete] {stage} WM snapshots {load} seed={seed} snapshots={metrics.get('decision_snapshots_saved')}")


def _run_label_cell(root: Path, load: str, seed: int) -> None:
    if int(seed) not in REPAIR_DATA_SEEDS:
        raise ValueError(f"long-risk seed must be in {list(REPAIR_DATA_SEEDS)}")
    ...
    _atomic_json(marker, {
        "schema_version": "station6_20x20_longrisk_label_run_v1",
        "output": output.as_posix(), "sha256": _sha256_file(output),
        "labels": len(labels), "horizon": LONG_RISK_HORIZON,
    })


def _run_snapshot_data_cell(root: Path, stage: str, load: str, seed: int) -> None:
    allowed_seeds = (
        CORE_DATA_SEEDS if stage == "core" else REPAIR_DATA_SEEDS if stage == "repair" else ()
    )
    if int(seed) not in allowed_seeds:
        raise ValueError(f"{stage} dataset seed must be in {list(allowed_seeds)}")
    ...
    _run(command, cwd=_repo_root(), log=root / "logs" / f"{stage}_snapshot_data_{load}_seed{seed}.log")
...
def _fuse_core(root: Path) -> Path:
    ...
    _atomic_json(root / "phasec_core_fusion_lineage.json", {
...
def _train_phasec_core(root: Path) -> Path:
    ...
    command = [
        ...
        "--splits", str(fused / "splits.json"),
        "--save-dir", str(output),
        "--skip-stage1", "--stage1-checkpoint", str(base),
        "--stage2-horizon", str(ROLLOUT_HORIZON), "--stage2-epochs", "30",
        "--stage2-lr", "1e-4", "--stage2-alpha-rank", "0.1",
        "--stage2-freeze-epochs", "0", "--stage2-unfreeze-mode", "all",
        "--stage2-val-dynamics-interval", "1", "--early-stopping-patience", "8",
    ...
    _run(command, cwd=_repo_root(), log=root / "logs" / "train_phasec_core.log", env_overrides=_cpu_training_env())
    checkpoint = output / "best_regret_world_model.pt"
    if not checkpoint.is_file():
        checkpoint = output / "world_model.pt"
    if not checkpoint.is_file():
    ...
    long_risk_keys = sorted(key for key in core_state if key.startswith("long_risk_head."))
    non_long_risk_keys = sorted(key for key in core_state if not key.startswith("long_risk_head."))
    cfg = core_payload.get("model_config") or {}
    action_schema = core_payload.get("action_schema") or {}
    checks = {
        "six_station_schema": (
            int(cfg.get("demand_dim", -1)) == 11 and int(cfg.get("num_stations", -1)) == STATION_COUNT
        ),
        "native_no_assign_schema": bool(action_schema.get("supports_no_assign_candidate"))
        and bool(action_schema.get("complete_group_coverage")),
        "long_risk_unsupervised_and_bitwise_preserved": bool(long_risk_keys)
        and all(torch.equal(base_state[key], core_state[key]) for key in long_risk_keys),
        "phase_c_non_head_training_occurred": any(
            not torch.equal(base_state[key], core_state[key]) for key in non_long_risk_keys
        ),
    }
    lineage = {
        "schema_version": "station6_20x20_phase_c_core_training_lineage_v1",
        "protocol_sha256": _load_protocol(root)[0]["protocol_sha256"],
...
def _fuse_longrisk(root: Path) -> Path:
    ...
    labels = [_label_path(root, load, seed) for load in LOADS for seed in REPAIR_DATA_SEEDS]
    ...
    _atomic_json(root / "longrisk_fusion_lineage.json", {
        "protocol_sha256": _load_protocol(root)[0]["protocol_sha256"],
        "inputs": [
            {"data": d.as_posix(), "labels": l.as_posix(), "data_sha256": _sha256_file(d), "labels_sha256": _sha256_file(l)}
...
def _train_longrisk(root: Path) -> Path:
    ...
    if checkpoint.is_file() and audit_path.is_file() and summary_path.is_file():
        ...
        if (
            ...
            and summary.get("output_checkpoint_sha256") == _sha256_file(checkpoint)
        ):
            return checkpoint
    ...
    _run(command, cwd=_repo_root(), log=root / "logs" / "train_longrisk.log", env_overrides=_cpu_training_env(max_threads=4))
    if not checkpoint.is_file():
...
def _station_dataset(root: Path) -> Path:
    """Build a state-level station-region latent dataset from WM snapshots."""
    ...
    train_rows = [row for row in records if row["seed"] in REPAIR_TRAIN_SEEDS]
    ...
    def split(seed: int) -> str:
        return "train" if seed in REPAIR_TRAIN_SEEDS else "val" if seed in REPAIR_VAL_SEEDS else "test"
    ...
    dataset = {
        ...
        "checkpoint_label_schema": "station6_20x20_state_level_current_v1",
        ...
        "arm_names": ["station6_20x20_adapted_wm"],
        "load_names": list(LOADS),
        "run_ids": [f"station6_20x20_repair_{load}_seed{seed}" for load in LOADS for seed in REPAIR_DATA_SEEDS],
        "splits": splits,
    }
    _atomic_torch_save(output, dataset)
    _atomic_json(output.parent / "dataset_summary.json", {
        "schema_version": "station6_20x20_state_head_dataset_summary_v1",
...
def _train_station_head(root: Path) -> Path:
    ...
    _run(command, cwd=_repo_root(), log=root / "logs" / "train_station_head.log", env_overrides=_cpu_training_env(max_threads=4))
    if not checkpoint.is_file():
        raise FileNotFoundError(checkpoint)
    return checkpoint
...
def _eval_cell(root: Path, arm: str, load: str, seed: int, ticks: int = TICKS) -> None:
    if int(seed) not in ONLINE_TEST_SEEDS:
        raise ValueError(f"online evaluation seed {seed} is outside the held-out block {ONLINE_TEST_SEEDS}")
    bundle, protocol = _load_protocol(root)
    _require_protocol_ticks(protocol, ticks)
    output = root / "evaluation" / "per_arm" / arm / f"{load}_seed{seed}.json"
    manifest = _eval_manifest_path(root, load, seed)
    expected_checkpoint = _train_longrisk(root) if arm in ("phasec", "combo_s1_j1") else None
    expected_head = _train_station_head(root) if arm == "combo_s1_j1" else None
    if output.is_file():
        ...
        resume_checks = {
            ...
            "manifest_hash": manifest.is_file() and meta.get("manifest_sha256") == _sha256_file(manifest),
            "checkpoint": expected_checkpoint is None or meta.get("checkpoint_sha256") == _sha256_file(expected_checkpoint),
            "station_head": expected_head is None or meta.get("station_head_sha256") == _sha256_file(expected_head),
        }
    ...
    if arm != "greedy" and not manifest_existed:
        raise FileNotFoundError(f"held-out Greedy manifest must be generated before {arm}: {manifest}")
    manifest.parent.mkdir(parents=True, exist_ok=True)
    from WorldModel.evaluation.evaluate_online_v6 import _run_one_assigner
    from WorldModel.evaluation.run_phase_c_s1_j1_long_risk_correction import _physical_only_audit_contract
    assigner = _make_assigner(arm, root)
    replay_kwargs = (
        {"recorded_orders_path": str(manifest)} if manifest_existed else {"save_order_manifest": str(manifest)}
    )
    with _physical_only_audit_contract() as holder:
        metrics = _run_one_assigner(
            str(_config(protocol, load)), assigner, int(seed), int(ticks),
            trace_label=f"Station6_20x20_{arm}", **replay_kwargs,
        )
    ...
    checks = {
        "manifest": metrics.get("order_arrival_manifest_sha256") == manifest_payload.get("manifest_sha256"),
        "manifest_count": int(metrics.get("order_arrival_count", -1)) == int(manifest_payload.get("total_orders", -2)),
        "ticks": int(metrics.get("ticks", -1)) == int(ticks),
        "robot_count": int(metrics.get("num_agents", -1)) == ROBOT_COUNT,
        "paired_replay": (not manifest_existed and arm == "greedy") or bool(metrics.get("order_arrival_replayed")),
        "physical_only": _physical_only_mode_ok(station_audit),
        "station_audit_passed": bool(station_audit.get("passed")),
        "physical_capacity_no_violation": int(station_audit.get("physical_capacity_violation_count", -1)) == 0,
        "no_committed_cap_contract": station_audit.get("committed_capacity_contract")
    ...
    if arm in ("phasec", "combo_s1_j1"):
        from WorldModel.core.long_risk_schema import LONG_RISK_SCHEMA_VERSION, long_risk_runtime_contract
        checks.update({
            "model_used": int(metrics.get("model_assign_calls", 0)) > 0,
            "no_greedy_fallback": int(metrics.get("fallback_greedy_calls", 0)) == 0,
            "long_risk_schema": metrics.get("long_risk_schema_version") == LONG_RISK_SCHEMA_VERSION,
            "long_risk_contract": metrics.get("long_risk_runtime_contract") == long_risk_runtime_contract(),
        })
    if arm == "phasec":
    ...
    if arm == "combo_s1_j1":
        checks.update({
            ...
            "j1_head_hash": metrics.get("psi_dispatch_head_checkpoint_sha256") == _sha256_file(expected_head),
            "j1_scale_hash": metrics.get("psi_dispatch_scale_contract_sha256")
            == _sha256_file(root / "station_head_dataset" / "station_congestion_scale_contract.json"),
            "j1_evaluated": int(metrics.get("psi_dispatch_eval_calls", 0)) > 0,
            "j1_contexts_seen": int(metrics.get("psi_dispatch_contexts_seen", 0)) > 0,
            "j1_parent_robot_scorer_preserved": metrics.get("psi_dispatch_robot_scorer")
            == "WorldModelTaskAssigner.select_robots_unmodified",
            "j1_encoder_binding": bool(metrics.get("psi_dispatch_encoder_contract_verified")),
            "j1_s1_variant": metrics.get("psi_dispatch_robot_scorer_variant") == "s1_within_context",
        })
    payload = {
        "schema_version": RUN_SCHEMA_VERSION,
        "meta": {
            ...
            "checkpoint": expected_checkpoint.as_posix() if expected_checkpoint is not None else None,
            "checkpoint_sha256": _sha256_file(expected_checkpoint) if expected_checkpoint is not None else None,
            "station_head": expected_head.as_posix() if expected_head is not None else None,
            "station_head_sha256": _sha256_file(expected_head) if expected_head is not None else None,
...
def _summarize(root: Path) -> None:
    ...
    summary = {
        ...
        "online_test_disjoint_from_all_adaptation_data": not bool(
            (set(CORE_DATA_SEEDS) | set(REPAIR_DATA_SEEDS)) & set(ONLINE_TEST_SEEDS)
        ),
        ...
        "audit": {
            "passed": (
                not bool((set(CORE_DATA_SEEDS) | set(REPAIR_DATA_SEEDS)) & set(ONLINE_TEST_SEEDS))
...
def _cells(mode: str) -> list[tuple[str, int]]:
    ...
    elif mode in ("collect_repair_snapshots", "label_longrisk", "build_repair_data"):
    ...
    with ProcessPoolExecutor(
        ...
        for load, seed in _cells(mode):
            if mode == "collect_base":
                ...
                future = pool.submit(_run_snapshot_cell, root, "core", load, seed, ticks)
            elif mode == "collect_repair_snapshots":
                future = pool.submit(_run_snapshot_cell, root, "repair", load, seed, ticks)
            elif mode == "label_longrisk":
                future = pool.submit(_run_label_cell, root, load, seed)
            elif mode == "build_core_data":
                future = pool.submit(_run_snapshot_data_cell, root, "core", load, seed)
            elif mode == "build_repair_data":
                future = pool.submit(_run_snapshot_data_cell, root, "repair", load, seed)
            else:
                raise ValueError(mode)
...
def _parallel_eval(root: Path, workers: int, ticks: int = TICKS) -> None:
    def run_batch(cells: Sequence[tuple[str, str, int]], label: str) -> None:
        ...
        with ProcessPoolExecutor(
            ...
            futures = {
                pool.submit(_eval_cell, root, arm, load, seed, ticks): (arm, load, seed)
    ...
    greedy_cells = [("greedy", load, seed) for load in LOADS for seed in ONLINE_TEST_SEEDS]
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=(
            ...
            "collect_core_snapshots", "build_core_data", "fuse_core", "train_core",
            "collect_repair_snapshots", "label_longrisk", "build_repair_data",
            "fuse_longrisk", "train_longrisk", "build_station_dataset",
            "train_station_head", "evaluate", "summarize", "all",
        ),
        required=True,
    )
    ...
    # Batch-capable collection modes: run all cells in parallel when no
    # explicit --seed is supplied (single cell otherwise).
    batch_collect = {
        "collect_base": lambda: _parallel_cells(root, "collect_base", args.workers, args.ticks),
        "collect_core_snapshots": lambda: _parallel_cells(root, "collect_core_snapshots", args.workers, args.ticks),
        "collect_repair_snapshots": lambda: _parallel_cells(root, "collect_repair_snapshots", args.workers, args.ticks),
        "label_longrisk": lambda: _parallel_cells(root, "label_longrisk", args.workers, args.ticks),
        "build_core_data": lambda: _parallel_cells(root, "build_core_data", args.workers, args.ticks),
        "build_repair_data": lambda: _parallel_cells(root, "build_repair_data", args.workers, args.ticks),
    }
    ...
    if args.mode == "prepare":
        ...
        if args.seed is None:
            batch_collect["collect_base"]()
        else:
            if args.load is None:
                parser.error("single collect_base cell requires --load with --seed")
        ...
        if args.seed is None:
            batch_collect["collect_core_snapshots"]()
        else:
            if args.load is None:
                parser.error("single collect_core_snapshots cell requires --load with --seed")
            _run_snapshot_cell(root, "core", args.load, args.seed, args.ticks)
    elif args.mode == "build_core_data":
        if args.seed is None:
            batch_collect["build_core_data"]()
        else:
            if args.load is None:
                parser.error("single build_core_data cell requires --load with --seed")
        ...
        if args.seed is None:
            batch_collect["collect_repair_snapshots"]()
        else:
            if args.load is None:
                parser.error("single collect_repair_snapshots cell requires --load with --seed")
            _run_snapshot_cell(root, "repair", args.load, args.seed, args.ticks)
    elif args.mode == "label_longrisk":
        if args.seed is None:
            batch_collect["label_longrisk"]()
        else:
            if args.load is None:
                parser.error("single label_longrisk cell requires --load with --seed")
            _run_label_cell(root, args.load, args.seed)
    elif args.mode == "build_repair_data":
        if args.seed is None:
            batch_collect["build_repair_data"]()
        else:
            if args.load is None:
                parser.error("single build_repair_data cell requires --load with --seed")
        ...
        if args.arm is None and args.seed is None:
            _parallel_eval(root, args.workers, args.ticks)
        else:
            if args.arm is None or args.load is None or args.seed is None:
                parser.error("single evaluate cell requires --arm/--load/--seed")
