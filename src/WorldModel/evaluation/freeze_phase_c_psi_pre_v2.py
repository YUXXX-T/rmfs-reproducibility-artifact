"""Freeze the independent, corrected ``psi_pre`` v2 protocol."""
...
FROZEN_FILENAME = "phase_c_psi_pre_v2_frozen_protocol.json"
...
def main() -> None:
    ...
    parser.add_argument(
        "--source-v1-root",
        type=Path,
        default=OUTPUT_ROOT.parent / "psi_pre_h10_531_540_v1",
    )
    ...
    protocol = formal_protocol(
        ...
        source_v1_root=args.source_v1_root,
    )
    ...
    bundle = {
        "schema_version": FROZEN_BUNDLE_SCHEMA_VERSION_V2,
        "protocol": protocol,
        ...
        "audit": {
            ...
            "node_bce_sigmoid_before_region_aggregation": True,
            "blocked_max_primary_penalty": False,
    ...
    _write_text_immutable(args.output_root / INPUTS_FILENAME, "\n".join(rows) + "\n")

    print(f"psi_pre v2 protocol sha256 = {protocol_sha}")
    print(f"source runs = {len(source_rows)}")
    print("primary channels = station_queue, assigned_load, region_density_mean, region_density_max")
    print("diagnostic channel = region_blocked_max (not a primary penalty)")
