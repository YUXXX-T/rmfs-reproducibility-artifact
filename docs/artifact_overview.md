...
- Exact four- and six-station simulator JSON configurations plus concise YAML contracts.
- All 150 main-evaluation paired arrival manifests and all 30 six-station held-out manifests.
- Flattened per-seed result rows with identity-bearing path fields removed.
...
`checkpoints/checkpoint_manifest.json` records expected release assets and paths. The 180 arrival files and their schema/order-count index make the paired offered load directly inspectable. The statistics-only path is complete without large files; full retraining and trace re-detection require the external bundle.
