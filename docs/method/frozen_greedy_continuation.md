...
For the model-owned long-risk head, this simulator continuation lasts 200 ticks; targets include peak/tail/terminal risk statistics. The documented joint world-model trainer can consume these targets alongside short-horizon labels. Complete behavior-continuation trajectories include future order generation and the continuation scheduler. Right-censored trajectories are rejected.
