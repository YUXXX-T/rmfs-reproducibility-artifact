...
For the model-owned long-risk head, this simulator continuation lasts 200 ticks; targets include peak/tail/terminal risk statistics. The canonical from-scratch trainer can consume these targets jointly with the short-horizon labels; the frozen evaluation lineage also contains a later head-only repair trained on them. Complete behavior-continuation trajectories include future order generation and the continuation scheduler. Right-censored trajectories are rejected.
